...
class TransactionService:
    ...
    async def create_transaction(
        ...
        if new_balance < 0:
            logger.warning(
                ...
                user_id=user_id,
                current_balance=Decimal(balance.amount),
                requested_amount=Decimal(transaction_data.amount),
            )
        ...
        logger.success(
            ...
            user_id=user_id,
            final_balance=Decimal(new_balance),
        )
    ...
    async def rollback_transaction(
        ...
        transaction = await self.transaction_repository.get_by_id_with_entries(transaction_id)
        if not transaction:
        ...
        if new_balance < 0:
            logger.warning(
                ...
                transaction_id=transaction_id,
                current_balance=Decimal(balance.amount),
                reverse_amount=Decimal(reverse_amount),
            )
            raise NegativeBalanceException(
                currency=str(transaction.currency),
                current_balance=Decimal(balance.amount),
                requested_amount=Decimal(reverse_amount),
            )
